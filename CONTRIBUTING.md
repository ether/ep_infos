# Contributing to ep_infos

Thanks for helping improve `ep_infos` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_infos` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_infos from your local clone
pnpm run plugins i --path ../ep_infos

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_infos/` are picked up after a server restart.
...
* Etherpad version, ep_infos version, browser + OS
* Steps to reproduce
